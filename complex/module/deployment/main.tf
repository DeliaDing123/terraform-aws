locals {
  ...
  lb_listener_by_name            = {for l in awscloud_clb_listener.lb_listeners           : l.listener_name => l}
  lb_listener_rule_by_lisener_id = {for r in awscloud_clb_listener.lb_listener_rules : r.listener_id   => r}
...
resource "awscloud_clb_listener" "lb_listeners" {
...
resource "awscloud_clb_listener" "lb_listener_rules" {
  count = length(var.deployment_plan.lb)
...
resource "awscloud_clb_attachment" "lb_attachments" {
...
resource "null_resource" "post_deployment_steps" {
  ...
  depends_on = [
    ...
    awscloud_clb_listener.lb_listeners,
    awscloud_clb_listener.lb_listener_rules,
    awscloud_clb_attachment.lb_attachments,
  ]
